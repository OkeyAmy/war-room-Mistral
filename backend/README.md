...
* **AI Integration:**
* **Mistral Large 3:** For text-based agent reasoning and scenario generation.
* **LiveKit SDK:** For real-time conversational streaming and interaction logic.
...
* Manages the complex orchestration of LiveKit sessions, voice configurations, and ElevenLabs voice assignments (`voice_assignment.py` & `voice_routes.py`).
...
3. **Active Simulation:** The user speaks or sends directives. The `chairman_handler.py` processes this, invokes the necessary `CrisisAgent` instances, which then query context, compute responses via Mistral Large 3, trigger TTS streaming via ElevenLabs, and broadcast the resulting audio/transcripts back through the WebSocket gateway.
