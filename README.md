...
graph TD
    ...
    VP <-->|Mistral Large 3| AI[Agent Array]
...
sequenceDiagram
    ...
    GW->>A: Stream to Mistral Large 3
...
**Purpose:**
The most complex interaction layer, designed to handle bidirectional voice streams via LiveKit between the user and the AI array.
...
* **Backend Transcription & Routing:** The backend receives the chunked audio, pipes it to the Mistral Large 3 for STT (Speech-to-Text) and intent reasoning, determines *which* AI agent should react, and triggers their specific logic loop.
...
* **`/backend/agents`:** Isolated Python classes inheriting from a `BaseCrisisAgent`. Each agent maintains its own localized prompt memory and connects to the Mistral SDK independently to reason about incoming inputs.
...
* Node.js (v18+)
* Python 3.10+
* Requested API Keys (Mistral Large 3, ElevenLabs, LiveKit) specified in `./backend/.env`
